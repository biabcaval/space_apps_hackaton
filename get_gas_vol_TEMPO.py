...
def get_lat_lon(path, state_sigla):
    df = pd.read_csv(path)
...
def get_poi_results(gas, date_start, date_end, state_sigla, version="V03"):
    ...
    POI_lon = get_lat_lon(state_sigla)[1]
...
lat, lon, strat_GAS_column, fv_strat_GAS, trop_GAS_column, fv_trop_GAS, GAS_unit, QF = (
    read_TEMPO_GAS_L3(granule_name)
)
...
def find_available_data(gas, start_date, end_date, state_sigla, max_days=30):
    """
...
def main(gas, state_sigla, start_date, end_date):
    found_date, POI_results = find_available_data(gas, start_date, end_date, state_sigla, max_days=30)
